...
import { Star, Trash2, Check, X, AlertCircle, Eye, Plus, Video, Music, FileText, Image as ImageIcon, Upload, Save, Edit } from 'lucide-react';
...
export default function TestimonialManagement() {
  ...
  const handleExport = () => {
    try {
      const headers = ['ID', 'Text', 'Rating', 'Media URL', 'Status', 'Primary', 'Created At'];
      const rows = testimonials.map((t) => [
        t.id || t._id || '',
        `"${(t.text || t.message || '').replace(/"/g, '""')}"`,
        t.rating || '',
        t.mediaUrl || '',
        t.status || '',
        t.primary ? 'Yes' : 'No',
  ...
  const statusBadge = (status) => {
    ...
    const displayText = s === 'PENDING' ? 'PRIMARY' : s;
    return (
  ...
  const stats = [
    ...
    { label: 'Primary', count: testimonials.filter(t => (t.status || '').toUpperCase() === 'PENDING').length, color: 'bg-yellow-100 text-yellow-800' },
    ...
    <div className="w-full">
      ...
      {/* <div className="flex gap-3 flex-wrap mb-6">
        {['all', 'pending', 'approved', 'rejected'].map((s) => (
          ...
          >
            {s === 'pending' ? 'Primary' : s.charAt(0).toUpperCase() + s.slice(1)}
          </button>
        ))}
      </div> */}
