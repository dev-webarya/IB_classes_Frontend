...
import { requestUserOTP, verifyUserOTP } from '../../api/api/api/authApi.js';
import { Card, Input, TextArea, Button } from '../../ui/index.jsx';
...
export const SubmitBlogPage = () => {
    ...
    const handleStep1 = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            await requestUserOTP(formData.authorEmail);
            toast.success('OTP sent to your email!'); setStep(2);
        } catch (err) { toast.error(getApiErrorMessage(err, 'Failed to send OTP. Please try again.')); }
    ...
    const handleStep2 = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault(); setLoading(true);
        try {
            await verifyUserOTP({ email: formData.authorEmail, otp });
            toast.success('Email verified!'); setStep(3);
        }
        catch (err) { toast.error(getApiErrorMessage(err, 'Invalid OTP. Please check your email.')); }
        finally { setLoading(false); }
    };
    ...
    const handleStep3 = async () => {
        ...
        try {
            ...
            clearDraft();
            // Clear user session token after blog submission (don't persist user login)
            localStorage.removeItem('icfy_user');
            if (!localStorage.getItem('icfy_role')) {
                localStorage.removeItem('icfy_token');
            }
            toast.success('Blog submitted successfully! Pending admin review.'); setStep(4);
        }
        catch (err) { toast.error(getApiErrorMessage(err, 'Failed to finalize. Please try again.')); }
        finally { setLoading(false); }
    };
