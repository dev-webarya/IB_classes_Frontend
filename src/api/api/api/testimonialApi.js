...
export const getApprovedTestimonials = async () => {
  try {
    ...
    // Fallback to localStorage - filter only approved testimonials
...
const STORAGE_KEY = 'icfy_admin_testimonials';
...
const setStoredTestimonials = (testimonials) => {
  try {
...
export const getAllTestimonials = async (params) => {
  try {
    ...
    const testimonials = getStoredTestimonials();
    return { content: testimonials, success: true };
  }
};
...
export const createTestimonialAdmin = async (data) => {
  try {
    ...
    setStoredTestimonials(testimonials);
    return { content: newTestimonial, success: true };
  }
};
...
export const rejectTestimonial = async (id) => {
  try {
    ...
    const testimonials = getStoredTestimonials();
    const updated = testimonials.map(t => 
      (t._id === id || t.id === id) ? { ...t, status: 'rejected' } : t
    );
    setStoredTestimonials(updated);
    return { success: true };
  }
...
export const deleteTestimonial = async (id) => {
  try {
    ...
    // Fallback to localStorage
    const testimonials = getStoredTestimonials();
    const filtered = testimonials.filter(t => t._id !== id && t.id !== id);
